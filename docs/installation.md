...
services:
  ...
  ocr:
    image: ghcr.io/nas-tools/nas-tools-ocr:latest
...
docker run -d \
    ...
    ghcr.io/nas-tools/nas-tools:latest

# OCR服务（可选，用于验证码识别）
docker run -d \
    --name nas-tools-ocr \
    -p 9300:9300 \
    ghcr.io/nas-tools/nas-tools-ocr:latest
...
docker run -d \
    ...
    -p 9850:9850 \
    --shm-size=2g `# 共享内存大小` \
    ghcr.io/nas-tools/nas-tools-chrome:latest
```
